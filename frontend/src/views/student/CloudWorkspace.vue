<template>
  <div class="ws-shell">
    ...
    <header class="ws-topbar">
      <button class="ws-btn ws-btn-ghost" title="??????" @click="goBack">
      ...
      <div class="ws-title-section">
        ...
        <span class="ws-title">{{ projectName || '????' }}</span>
      </div>
      <div class="ws-topbar-right">
        <span v-if="fileContentDirty" class="ws-unsaved">???</span>
        <button v-if="activePath" class="ws-btn ws-btn-outline ws-btn-sm" @click="saveFile" :disabled="savingFile">
          {{ savingFile ? '???...' : '??' }}
        </button>
      </div>
    </header>
    ...
    <div class="ws-body">
      ...
      <aside class="ws-sidebar">
        <div class="ws-sidebar-header">
          <span>???????</span>
          <button class="ws-btn ws-btn-ghost ws-btn-sm" title="??" @click="loadRoot">
        ...
        <div class="ws-tree" v-loading="treeLoading">
          <FileTreeNode
            ...
            @select="openFile"
          />
          <div v-if="!treeLoading && fileTree.length === 0" class="ws-tree-empty">
            ????
          </div>
        </div>
      </aside>
      ...
      <main class="ws-editor">
        <div v-if="!activePath" class="ws-editor-empty">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#e5e7eb" stroke-width="1">
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
            <polyline points="14 2 14 8 20 8"/>
          </svg>
          <p>????????</p>
          <p class="ws-editor-hint">?????????????</p>
        </div>
        <template v-else>
      ...
      <aside class="ws-agent">
        <div class="ws-agent-header">
          ...
          <button class="ws-btn ws-btn-ghost ws-btn-sm" title="????" @click="messages = []">
        ...
        <div class="ws-agent-messages" ref="msgContainer">
          ...
          <TransitionGroup name="msg" tag="div" class="ws-msg-list">
            <div v-for="(msg, i) in messages" :key="i" class="ws-msg" :class="msg.role">
              <div class="ws-msg-bubble">
                <div class="ws-msg-text">{{ msg.content }}</div>
              </div>
            </div>
          </TransitionGroup>
          <div v-if="agentLoading" class="ws-msg assistant">
            <div class="ws-msg-bubble">
        ...
        <div class="ws-agent-input">
          ...
          />
          <button
            class="ws-btn ws-btn-primary ws-btn-sm ws-send"
            @click="sendMessage"
...
const langMap = {
  ...
  vue: 'html', py: 'python', java: 'java', c: 'c', cpp: 'cpp',
  html: 'html', css: 'css', scss: 'scss', json: 'json', xml: 'xml',
  yml: 'yaml', yaml: 'yaml', md: 'markdown', sql: 'sql', sh: 'shell',
...
async function loadRoot() {
  ...
  try {
    const r = await projectApi.getTree(projectId.value, '')
...
async function loadChildren(dirPath) {
  if (!projectId.value) return []
  try {
    const r = await projectApi.getTree(projectId.value, dirPath)
    return r.data || []
  } catch (e) {
    return []
  }
...
async function saveFile() {
  ...
  try {
    await projectApi.saveFile(projectId.value, activePath.value, fileContent.value)
    fileContentDirty.value = false
    ElMessage.success('?????')
  } catch (e) {
    ElMessage.error('????')
...
async function sendMessage() {
  ...
  scrollDown()
  try {
    const r = await projectApi.askAgent(projectId.value, {
...
function goBack() {
  router.push({ name: 'StudentCloudSpace' })
}
</script>
