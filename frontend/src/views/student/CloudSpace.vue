    ...
    <div class="cs-left">
      ...
      <div class="cs-list" v-loading="listLoading">
        <div v-if="!listLoading && projects.length === 0" class="cs-empty">
          ...
          <p>暂无项目</p>
          <p class="cs-empty-hint">点击上方新建或上传项目</p>
        </div>
        <TransitionGroup name="proj-list" tag="div">
          <div v-for="proj in projects" :key="proj.projectId" class="cs-item" :class="{ active: selectedId === proj.projectId }" @click="selectProject(proj)">
            ...
            <div class="cs-item-actions">
              <button class="cs-btn cs-btn-ghost cs-btn-sm" :title="'进入工作空间'" @click.stop="enterWorkspace(proj)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg></button>
    ...
    <div class="cs-right">
      ...
      <div v-else class="cs-right-panel">
        <div class="cs-panel-header cs-right-header">
          <div class="cs-right-title"><h3>{{ selectedProject.projectName }}</h3><span class="cs-right-meta">{{ selectedProject.fileCount || 0 }} 个文件</span></div>
        ...
        <div class="cs-tree-panel" v-loading="treeLoading">
          ...
          <div v-if="!treeLoading && fileTree.length === 0" class="cs-empty" style="padding:24px"><p>空项目</p></div>
        </div>
      </div>
    </div>
    <Teleport to="body">
        ...
        <div v-if="showCreate" class="cs-overlay" @click.self="showCreate = false">
          <div class="cs-modal">
            ...
            <div class="cs-template-section">
              <p class="cs-template-label">项目模板（可选）</p>
...
function enterWorkspace(proj) { if (!proj || !proj.projectId) return; router.push({ name: 'StudentCloudWorkspace', params: { projectId: proj.projectId } }) }
async function createProject() { const name = newProjectName.value.trim(); if (!name) { ElMessage.warning('请输入项目名称'); return }; try { if (selectedTemplate.value) { await projectApi.createWithTemplate(name, selectedTemplate.value) } else { await projectApi.createEmpty(name) }; ElMessage.success('项目创建成功'); showCreate.value = false; newProjectName.value = ''; selectedTemplate.value = null; await loadProjects() } catch (e) { ElMessage.error('创建失败') } }
function triggerUpload() { uploadInput.value?.click() }
async function handleUpload(e) { const f = e.target.files[0]; if (!f) return; try { await projectApi.upload(f); ElMessage.success('上传成功'); await loadProjects() } catch (e) { ElMessage.error('上传失败') } finally { e.target.value = '' } }
async function deleteProject(proj) { try { await ElMessageBox.confirm('确定要删除项目 \"' + proj.projectName + '\" \uff1f', '确认删除', { confirmButtonText: '确定删除', cancelButtonText: '取消', type: 'warning' }); await projectApi.delete(proj.projectId); ElMessage.success('删除成功'); if (selectedId.value === proj.projectId) { selectedProject.value = null; selectedId.value = null; fileTree.value = [] }; await loadProjects() } catch (e) { if (e !== 'cancel' && e !== 'close') { ElMessage.error('删除失败') } } }
