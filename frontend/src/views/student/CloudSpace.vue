      ...
      <div class="cs-list" v-loading="listLoading">
        ...
        <TransitionGroup name="proj-list" tag="div">
          <div
            ...
            :key="proj.projectId"
            class="cs-item"
            :class="{ active: selectedId === proj.projectId }"
          ...
          >
            ...
            <div class="cs-item-actions">
              <button class="cs-btn cs-btn-ghost cs-btn-sm" title="??????" @click.stop="enterWorkspace(proj)">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>
              </button>
              <button class="cs-btn cs-btn-ghost cs-btn-sm cs-btn-danger" title="????" @click.stop="deleteProject(proj)">
      ...
      <div v-else class="cs-right-panel">
        <div class="cs-panel-header cs-right-header">
          <div class="cs-right-title">
            ...
            <span class="cs-right-meta">{{ selectedProject.fileCount || 0 }} ???</span>
          </div>
...
async function deleteProject(proj) {
  try {
    await ElMessageBox.confirm('??????? "' + proj.projectName + '" ??', '????', {
      confirmButtonText: '????',
      cancelButtonText: '??',
      type: 'warning'
    })
...
// onActivated for keep-alive support
...
.cs-template-item:hover {
  border-color: #6366f1;
  background: #f5f5ff;
}
.cs-template-item.active {
  border-color: #6366f1;
  background: #eef2ff;
...
.modal-enter-from .cs-modal,
.modal-leave-to .cs-modal {
